    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
                <StatCard 
                    label="Total Line" 
                    value={(stats?.totalLine || 0).toLocaleString('id-ID')} 
                    icon={Users} 
                    color="bg-[#FF006E]" 
                />
                <StatCard 
                    label="Total Operator" 
                    value={(stats?.totalOperator || 0).toLocaleString('id-ID')} 
                    icon={GraduationCap} 
                    color="bg-[#FB5607]" 
                />
                <StatCard 
                    label="Total Gudang" 
                    value={(stats?.totalGudang || 0).toLocaleString('id-ID')} 
                    icon={Store} 
                    color="bg-[#3A86FF]" 
                />
                <StatCard 
                    label="SKU Aktif" 
                    value={(stats?.totalSku || 0).toLocaleString('id-ID')} 
                    icon={BookOpen} 
                    color="bg-[#8338EC]" 
                />
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 lg:grid-cols-12 gap-2">
                ...
                <div className="lg:col-span-6 bg-white p-8 rounded border border-slate-100 shadow-sm">
                    <div className="flex items-center justify-between mb-2">
                        <h4 className="text-md  text-slate-700 font-semibold uppercase tracking-tight">Tren Output Produksi <span className="text-slate-400 italic capitalize"> (7 Hari Terakhir)</span></h4>
                    </div>
                    <CustomLineChart data={stats?.trendProduksi || []} />
                </div>

                {/* Middle: Donut 1 */}
                <div className="lg:col-span-3 bg-white p-8 rounded border border-slate-100 shadow-sm flex flex-col justify-between">
                    <h4 className="text-md text-slate-700 font-semibold uppercase tracking-tight mb-8">Order Produksi <span className="text-slate-400 italic capitalize"> (Hari Ini)</span></h4>
                    <CustomDonutChart 
                        percentage={stats?.produksiHariIni?.total > 0 ? Math.round(((stats.produksiHariIni.byStatus['selesai'] || 0) / stats.produksiHariIni.total) * 100) : 0} 
                        color="#10b981" 
                        label="SELESAI" 
                    />
                    <div className="mt-8 space-y-3">
                        {[
                            { label: 'Terjadwal', key: 'terjadwal', color: 'bg-indigo-500' },
                            ...
                            <div key={item.label} className="flex items-center justify-between text-xs">
                                ...
                                <span className="text-slate-800">{stats?.produksiHariIni?.byStatus?.[item.key] || 0}</span>
                            </div>
                        ))}
                        <div className="pt-3 border-t border-slate-50 flex items-center justify-between">
                            <span className="text-xs text-slate-800 uppercase font-medium">Total</span>
                            <span className="text-xs text-slate-800 font-medium">{(stats?.produksiHariIni?.total || 0).toLocaleString('id-ID')}</span>
                        </div>
                    </div>
                </div>
                ...
                {/* Right: Donut 2 */}
                <div className="lg:col-span-3 bg-white p-8 rounded border border-slate-100 shadow-sm flex flex-col justify-between">
                    <h4 className="text-md text-slate-700 font-semibold uppercase tracking-tight mb-8">Pengiriman Harian <span className="text-slate-400 capitalize italic"> (Hari Ini)</span></h4>
                    <CustomDonutChart 
                        percentage={stats?.pengirimanHariIni?.total > 0 ? Math.round(((stats.pengirimanHariIni.byCategory['terkirim'] || 0) / stats.pengirimanHariIni.total) * 100) : 0} 
                        color="#10b981" 
                        label="TERKIRIM" 
                    />
                    <div className="mt-8 space-y-3">
                        {stats?.pengirimanHariIni?.byCategory && Object.keys(stats.pengirimanHariIni.byCategory).length > 0 ? (
                            Object.entries(stats.pengirimanHariIni.byCategory).slice(0, 4).map(([category, count], index) => {
                            ...
                            <div className="flex items-center justify-center py-4 text-xs text-slate-400">Belum ada pengiriman</div>
                        )}
                        <div className="pt-3 border-t border-slate-50 flex items-center justify-between">
                            <span className="text-xs text-slate-800 uppercase font-medium">Total Pengiriman</span>
                            <span className="text-xs text-slate-800 font-medium">{(stats?.pengirimanHariIni?.total || 0).toLocaleString('id-ID')}</span>
                        </div>
                    </div>
                </div>
